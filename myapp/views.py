...
def randompredict(request):
    ...
    if request.method=="POST":
        ...
        model = load_model('model/G_or_R_LSTM_model.h5')

        # Load new data for prediction (modify this part according to your actual data)
        # Check if file was uploaded
        if 'myFile' in request.FILES:
             new_data = pd.read_csv(request.FILES['myFile'])
        else:
             # Fallback or error - for now keeping logic but using relative path if intended as default
             new_data = pd.read_csv('Good_or_Ransomware_model_data/X_test_G_OR_R.csv')
        ...
        result_df.to_csv('Good_or_Ransomware_model_data/G_or_B_prediction_results.csv', index=False)
        inputdf=pd.read_csv('Good_or_Ransomware_model_data/G_or_B_prediction_results.csv')
        # Display the results
        print("Predictions:")
        print(result_df)
        # Display the results
        ...
        print(res)
        with open('Good_or_Ransomware_model_data/G_or_B_prediction_results.csv', 'r') as csv_file:
...
def rfamilypredict(request):
    if request.method == 'POST':
        ...
        if 'myFile' in request.FILES:
             ...
             new_data = pd.read_csv('Ransomware_Family_model_data/X_test_Family.csv')
